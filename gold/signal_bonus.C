...
int main(int argc, char * argv[]){
  ...
  for (Long64_t i = 0; i < Nsim; i++){
    ...
    if (weight > 0){
      ...
      fclas12fa = 0;//CheckCLAS12FA(&P_p, "p", z);
      fclas12la = 0;//CheckCLAS12LA(&P_p, "p", z);
      if (fbonus > 0.0)
	...
	DetectorResolutionSmear(&P_p, res_clas12la);
      ...
      fclas12fa = 0;//CheckCLAS12FA(&P_Kp, "K+", z);
      fclas12la = 0;//CheckCLAS12LA(&P_Kp, "K+", z);
      if (fbonus > 0.0)
	...
	DetectorResolutionSmear(&P_Kp, res_clas12la);
      ...
      fclas12fa = 0;//CheckCLAS12FA(&P_Km, "K-", z);
      fclas12la = 0;//CheckCLAS12LA(&P_Km, "K-", z);
      if (fbonus > 0.0)
  ...
  //h4->Draw("same");
  leg->Draw("same");
  cc->Print(Form("signal_background_bonus_e%.1f.pdf", Ebeam));
  ...
  //h4p->Draw("same");
  legp->Draw("same");
  ccp->Print(Form("signal_background_momentemcut_bonus_e%.1f.pdf", Ebeam));
  ...
  //h4s->Draw("same");
  legs->Draw("same");
  ccs->Print(Form("signal_background_smear_bonus_e%.1f.pdf", Ebeam));
  ...
  //h4sp->Draw("same");
  legsp->Draw("same");
  ccsp->Print(Form("signal_background_smear_momentumcut_bonus_e%.1f.pdf", Ebeam));
