...
int main(int argc, char * argv[]){
  ...
  for (Long64_t i = 0; i < Nsim; i++){
    weight = GeneratePhotoproductionBoundStateCarbon(ki, kf);
    //weight = GenerateEvent_NKKN_withPhotoproductionBoundStateCarbon(ki, kf);

    if (weight > 0){
      h0->Fill(0.5, weight * 132.0);
    }
  }
