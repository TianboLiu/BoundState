...
int main(int argc, char * argv[]){
  ...
  SetTagger(0.5, Ebeam, atof(argv[2]) * degtorad, atof(argv[3]) * degtorad);
  //SetTagger(0.5, 4.5);
  ...
  for (Long64_t i = 0; i < Nsim; i++){
    ...
    //channel 1
    weight = GenerateEvent_eNKK_withElectroproductionPhiCarbon(ki, kf);
    if (weight > 0){
      P0_p = kf[1];
      P0_Kp = kf[2];
      P0_Km = kf[3];
    ...
    //channel 2
    weight = GenerateEvent_eNKK_withElectroproductionLambda1520Carbon(ki, kf);
    if (weight > 0){
      P0_p = kf[1];
      P0_Kp = kf[2];
      P0_Km = kf[3];
    ...
    //channel 3
    weight = GenerateEvent_eNKK_withElectroproductionKKCarbon(ki, kf);
    if (weight > 0){
      P0_p = kf[1];
      P0_Kp = kf[2];
      P0_Km = kf[3];
