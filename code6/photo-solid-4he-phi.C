...
int main(const int argc, const char * argv[]){
  ...
  TFile * fraw = new TFile("result-photon-phi/He4raw.root", "RECREATE");
  ...
  TFile * fsub = new TFile("result-photon-phi/He4sub.root", "RECREATE");
  ...
  for (Long64_t i = 0; i < Nsim; i++){
    ...
    weight *= GENERATE::Event_g4He2ee_Phi(ki, kf);

    weight *= (DETECTOR::AcceptanceCLAS12FD(kf[0], "e+") + DETECTOR::AcceptanceCLAS12CD(kf[0], "e+"))
      * (DETECTOR::AcceptanceCLAS12FD(kf[1], "e-") + DETECTOR::AcceptanceCLAS12CD(kf[1], "e-"));

    if (weight > 0.0){

      hMPhi1->Fill( (kf[0]+kf[1]).M(), weight);
      ...
      hThetaPphi1->Fill( (kf[0]+kf[1]).Theta()/M_PI*180, (kf[0]+kf[1]).P(), weight);
	
      if (ki[0].E() < 1.57){

	hMPhi2->Fill( (kf[0]+kf[1]).M(), weight);
	...
	hThetaPphi2->Fill( (kf[0]+kf[1]).Theta()/M_PI*180, (kf[0]+kf[1]).P(), weight);      
      }
    }
