...
int main(int argc, char * argv[]){
  ...
  double res_bonus[3] = {0.10, 0.020, 0.020};//resolution dp/p, dth, dphi
  ...
  for (Long64_t i = 0; i < Nsim; i++){
    ...
    if (weight0 > 0){
      ...
      P_Km = kf0[3];
      //proton detection
      fbonus = CheckProtonBONUS(&P_p, z);
      fclas12fa = CheckProtonCLAS12FA(&P_p, z);
      fclas12la = CheckProtonCLAS12LA(&P_p, z);
      if (fbonus > 0.0)
	...
	DetectorResolutionSmear(&P_p, res_clas12la);
      ...
      //K+ detection
      fbonus = CheckKaonBONUS(&P_Kp, z);
      fclas12fa = CheckKaonCLAS12FA(&P_Kp, z);
      fclas12la = CheckKaonCLAS12LA(&P_Kp, z);
      if (fbonus > 0.0)
	...
	DetectorResolutionSmear(&P_Kp, res_clas12la);
      ...
      //K- detection
      fbonus = CheckKaonBONUS(&P_Km, z);
      fclas12fa = CheckKaonCLAS12FA(&P_Km, z);
      fclas12la = CheckKaonCLAS12LA(&P_Km, z);
      if (fbonus > 0.0)
	...
	h1->Fill(P0_all.M(), weight1 * ftotal * A / 2.0);

	if (MomentumCut(&P0_p, 0.0, 0.6) && MomentumCut(&P0_Kp, 0.0, 0.4) && MomentumCut(&P0_Km, 0.0, 0.4))
	  h1p->Fill(P0_all.M(), weight1 * ftotal * A / 2.0);

	h1s->Fill(P_all.M(), weight1 * ftotal * A / 2.0);

	if (MomentumCut(&P_p, 0.0, 0.6) && MomentumCut(&P_Kp, 0.0, 0.4) && MomentumCut(&P_Km, 0.0, 0.4))
	  h1sp->Fill(P_all.M(), weight1 * ftotal * A / 2.0);
      }
    }
	...
	h2->Fill(P0_all.M(), weight2 * ftotal * A / 2.0);

	if (MomentumCut(&P0_p, 0.0, 0.6) && MomentumCut(&P0_Kp, 0.0, 0.4) && MomentumCut(&P0_Km, 0.0, 0.4))
	  h2p->Fill(P0_all.M(), weight2 * ftotal * A / 2.0);

	h2s->Fill(P_all.M(), weight2 * ftotal * A / 2.0);

	if (MomentumCut(&P_p, 0.0, 0.6) && MomentumCut(&P_Kp, 0.0, 0.4) && MomentumCut(&P_Km, 0.0, 0.4))
	  h2sp->Fill(P_all.M(), weight2 * ftotal * A / 2.0);
      }
    }
	...
	h3->Fill(P0_all.M(), weight3 * ftotal * A / 2.0);

	if (MomentumCut(&P0_p, 0.0, 0.6) && MomentumCut(&P0_Kp, 0.0, 0.4) && MomentumCut(&P0_Km, 0.0, 0.4))
	  h3p->Fill(P0_all.M(), weight3 * ftotal * A / 2.0);

	h3s->Fill(P_all.M(), weight3 * ftotal * A / 2.0);

	if (MomentumCut(&P_p, 0.0, 0.6) && MomentumCut(&P_Kp, 0.0, 0.4) && MomentumCut(&P_Km, 0.0, 0.4))
	  h3sp->Fill(P_all.M(), weight3 * ftotal * A / 2.0);
      }
    }
