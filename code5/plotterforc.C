...
if (opt == -401){//invariant mass cut
    TFile * fs = new TFile(loadpath + "cuttest.root", "r");//
    ...
    hB->SetMaximum(0.055/*h0->GetMaximum() * 1.2*/);
    ...
    h5->DrawClone("same");
    leg->DrawClone("same");
    c0->Print(savepath + "Massfinalcut.pdf(","pdf");//
   ...
   /* 
    ...
    hB->GetXaxis()->SetTitle("Theta_(pK^{+}) (rad)");
    hB->DrawClone("axis");
    h0d->DrawClone("same");
    h5d->DrawClone("same");
    leg->Clear();
    ...
    hB->SetMaximum(20.0);
    hB->SetAxisRange(0.0, M_PI, "X");
    hB->GetXaxis()->SetTitle("Theta_(K^{+}K^{-}) (rad)");
    hB->DrawClone("axis");
    h0f->DrawClone("same");
    h5f->DrawClone("same");
    leg->Clear();
 ...
 */   
  ...
  if (opt == -411){//Momentum cut
    ...
    leg->AddEntry(g5a, "misidentified #pi#pi", "p");
    TCanvas * c0 = new TCanvas("c0", "", 800, 600);
    ...
    g5a->DrawClone("psame");
    leg->DrawClone("same");
    c0->Print(savepath + "Momentumfinalcut.pdf(","pdf");//
  ...
  if (opt == -421){//P-theta cut
    ...
    rB->DrawClone("axis");
    r0a->Draw("colsame");
    c0->Print(savepath + "PThetafinalcut.pdf(", "pdf");
