...
int main(const int argc, const char * argv[]){
  ...
  if (argc < 2) {
    cout << "./smeared <savepath> <Nsim>" << endl;
    return 0;
  }

  TString path = argv[1];
  TString filename = path + "smeared-photon.root";
  ...
  for (Long64_t i = 0; i < Nsim; i++){
    ...
    if (weight > 0){
      ...
      d1a->Fill(kf[1].P(), kf[0].P(), weight);
      d1b->Fill(kf[2].P(), kf[0].P(), weight);
      d1c->Fill(kf[1].P(), kf[2].P(), weight);
      r1a->Fill(kf[0].Theta() * deg, kf[0].P(), weight);
